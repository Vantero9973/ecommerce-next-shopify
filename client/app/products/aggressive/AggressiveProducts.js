...
const pages = [
  ...
  { name: "Aggressive", href: "/products/aggressive", current: true },
];
...
export default function FilteredProducts({ aggressiveProducts }) {
  const [mobileFiltersOpen, setMobileFiltersOpen] = useState(false);
  const [selectedBrands, setSelectedBrands] = useState([]);
  const [selectedPrice, setSelectedPrice] = useState([]);

  const filterProducts = () => {
    return aggressiveProducts.filter((product) => {
      const brandMatch =
